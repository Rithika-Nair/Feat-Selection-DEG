...
# Creating target df
samples <- rownames(colData(mae_tcga_kipan))
response <- factor(colData(mae_tcga_kipan)$response, levels = levels(colData(mae_tcga_kipan)$response)) 
target <- data.frame(samples = samples, response = response)

# Prepare data 
x <- as.matrix(kipan_assay_df)  # Genes as features
...
# Selecting only genes found in protein half lives list
selected_genes <- intersect(colnames(x), df_hl$Human.gene.name)
...
# Protein half lives
quantile_proteins <- extreme_living_proteins_datasets
...
for (alpha_val in alpha_values) {
  ...
  ###CONTROL CONDITION: Full Gene Set (Top 100% of genes from proteins list)
  ...
  shortest_genes <- intersect(colnames(x), quantile_proteins[["5%_Shortest"]]$Human.gene.name)
  ...
  longest_genes <- intersect(colnames(x), quantile_proteins[["5%_Longest"]]$Human.gene.name)
  ...
  no_middle_genes <- intersect(colnames(x), c(quantile_proteins[["2.5%_Longest"]]$Human.gene.name, 
                                              quantile_proteins[["2.5%_Shortest"]]$Human.gene.name))
